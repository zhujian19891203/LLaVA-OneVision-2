...
#
# Sets BOTH OFFLINE_PACKING_BMR=1 and OFFLINE_PACKED_DATA=1.
#
# Required because (verified pretrain_llava_onevision2.py + task_encoder.py):
#   - OFFLINE_PACKING_BMR=1 -> per sub-sample encode via MultiMixQASample
#     (multi-turn aware; correct text/labels per sub-sample)
#   - OFFLINE_PACKED_DATA=1 -> batch() reads real cu_lengths/max_lengths,
#     LLM forward gets PackedSeqParams(qkv_format="thd", cu_seqlens_q=...)
#     so flash-attn varlen kernel enforces causal attention WITHIN each
#     sub-sample and zero attention across sub-sample boundaries.
# =============================================================================
